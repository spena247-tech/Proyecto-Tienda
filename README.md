...
├── css/styles.css      → toda la identidad visual (colores, tipografía, layout)
├── js/script.js        → datos del catálogo, tiendas, filtros, carrito y enlaces de WhatsApp
└── assets/favicon.svg  → ícono de la marca
```

## Qué personalizar primero

Todo lo editable está centralizado en **`js/script.js`**, marcado con `⚙️ CONFIGURA AQUÍ`:
